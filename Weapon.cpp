...
Weapon::Weapon(float power, float bulletMass, uint damage, float reload, float rate, float accuracy, float recoil, float bulletSize, uint capacity, const Animator& weaponAnimator, const Animator& bulletAnimator) :
        angle(0), power(power), bullet_mass(bulletMass), damage(damage), reload_time(reload), rate(rate), accuracy(accuracy), recoil(recoil),
...
Weapon::Weapon(const Weapon& temp) :
        ...
        reload_time(temp.reload_time), rate(temp.rate), accuracy(temp.accuracy), recoil(temp.recoil),
